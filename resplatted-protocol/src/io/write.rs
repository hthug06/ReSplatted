...
impl<W: BufMut> MinecraftWriteExt for W {
...
mod tests {
    ...
    fn test_write_varint_max() {
        ...
        assert_eq!(buf.as_ref(), &[0xFF, 0xFF, 0xFF, 0xFF, 0x07]);
    }
    ...
    fn test_write_varint_negative() {
        let mut buf = BytesMut::new();
        buf.write_var_int(-1);
        assert_eq!(buf.as_ref(), &[0xFF, 0xFF, 0xFF, 0xFF, 0x0F]);
    }
    ...
    fn test_write_string_simple() {
        ...
        assert_eq!(buf.as_ref(), &[0x05, b'H', b'e', b'l', b'l', b'o']);
    }
